...
const nav = [{ href: "#overview", label: "项目概述" },{ href: "#profile", label: "用户画像" },{ href: "#journey", label: "销售流程" },{ href: "#persona", label: "用户的一天" },{ href: "#findings", label: "核心问题" },{ href: "#needs", label: "需求定义" },{ href: "#demo", label: "方案 Demo" },{ href: "#archive", label: "访谈整理" }];
...
function Label({n,children}:{n:string;children:React.ReactNode}){
  const labels: Record<string, string> = { "项目概览": "项目概述", "用户画像": "用户画像", "访谈发现": "核心问题", "需求方向": "需求定义", "原始资料": "访谈整理" };
...
<section id="profile" className="profile-section section"><Label n="01">用户画像</Label><div className="profile-image-wrap"><img className="profile-image" src="assets/user-persona.png" alt="用户画像：初入职场的新生牛马，小田，27岁，杭州，线上保险销售，1年工作经验" /></div></section>
...
<section id="persona" className="dark section"><Label n="03">用户的一天</Label><div className="user-day-video"><video controls playsInline preload="metadata" poster="assets/user-day-poster.jpg" aria-label="小安的一天视频"><source src="assets/user-day-latest.mp4" type="video/mp4" />你的浏览器暂不支持视频播放。</video></div></section>
<section id="findings" className="paper section"><Label n="04">访谈发现</Label><div className="heading"><h2>问题不在某一个点，<br/><em>而在整条链路。</em></h2><p>时间不够、客户难找、信任难建、信息难记，以及对平台规则和 AI 的不确定，是访谈中反复出现的关键词。</p></div><div className="pain-grid">{pains.map(([n,t,d])=><article key={n}><span>{n}</span><h3>{t}</h3><p>{d}</p></article>)}</div><InterviewFragments /></section>
<NeedsSection />
<section id="demo" className="demo-section section"><Label n="06">方案 Demo</Label><div className="demo-heading"><h2>从访谈洞察，<br/><em>走向可体验的方案。</em></h2><p>这里会持续收录基于访谈洞察构建的方案原型。</p></div><div className="demo-grid"><a className="demo-card" href="https://cain0624.github.io/taiping-sdr-workbench/" target="_blank" rel="noreferrer"><span className="demo-icon">AI</span><span className="demo-card-copy"><b>太平洋 SDR 工作台</b><small>销售辅助方案 Demo</small></span><span className="demo-arrow">↗</span></a></div></section>
...
<section id="archive" className="archive"><Label n="07">原始资料</Label><ArchiveGallery /></section>
</main>}
